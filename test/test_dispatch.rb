...
class DispatchTest < BotTest
  ...
  # A duplicate must still be ACKed so Slack stops resending
  def test_duplicate_event_is_acked_but_not_reprocessed
    connect
    @slack.deliver(events_msg(event_id: "Ev1", envelope_id: "e1"))
    pump
    @slack.deliver(events_msg(event_id: "Ev1", envelope_id: "e2", retry_attempt: 1))
    pump
    assert_equal 1, @calls.size
    assert_equal [{ envelope_id: "e1" }, { envelope_id: "e2" }], @slack.acks
  end
